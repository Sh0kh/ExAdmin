...
export default function UncheckedExam() {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const response = await $api.get(`/teacher/unchecked-exams`,)
            ...
            {data?.length > 0 ? (
                ...
                <div className="flex items-center justify-center min-h-[600px]">
                    <h1 className="text-[25px] font-bold">
                        Hozircha tekshirilmagan imtihonlar yo‘q
                    </h1>
                </div>
            )}
        </div>

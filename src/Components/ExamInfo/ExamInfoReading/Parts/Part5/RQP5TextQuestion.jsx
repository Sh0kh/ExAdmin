...
export default function RQP5TextQuestion({ Edit, Delete, data }) {
    ...
    return (
        <div className="flex items-start justify-start gap-[15px] flex-col mt-[20px]">
            <h1 className="text-[30px] font-bold text-MainColor">Savolar</h1>
            {data?.questions
                ?.filter((_, index) => index !== 0) // Исключаем первый элемент
                .map((question, index) => (
                    <Accordion
                        key={question.id}
                        open={open === index}
